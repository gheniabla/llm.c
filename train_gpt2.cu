...
void gpt2_forward(GPT2 *model, const int* inputs, size_t B, size_t T, int step, float* coord_check_data) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        if (should_coord_check) {
            coord_check_data[cnt++] = get_mean_l1_summary(l_attproj, B*T*C);
        }
        fused_residual_forward5(l_residual2, l_ln2, l_ln2_mean, l_ln2_rstd, residual, l_attproj, l_ln2w, l_ln2b, B*T, C, 0., 0., main_stream);
        if (should_coord_check) {
            coord_check_data[cnt++] = get_mean_l1_summary(l_residual2, B*T*C);
        }
        matmul_forward_cublaslt(l_fch, l_ln2, l_fcw, l_fcb, B, T, C, 4*C, main_stream);
        if (should_coord_check) {
            coord_check_data[cnt++] = get_mean_l1_summary(l_fch, B*T*4*C);
        }
        gelu_forward(l_fch_gelu, l_fch, B*T*4*C, main_stream);
        if (should_coord_check) {
            coord_check_data[cnt++] = get_mean_l1_summary(l_fch_gelu, B*T*4*C);
        }
        matmul_forward_cublaslt(l_fcproj, l_fch_gelu, l_fcprojw, l_fcprojb, B, T, 4*C, C, main_stream);
        if (should_coord_check) {
            coord_check_data[cnt++] = get_mean_l1_summary(l_fcproj, B*T*C);
        }
